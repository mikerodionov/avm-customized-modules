variable "containers" {
  ...
  validation {
    # Security measure: Prevent/block "Azure storage account has a blob container with public access"
    condition     = alltrue([for container in var.containers : container.public_access == "None"])
    error_message = "Public access must be set to 'None' for all containers."
  }
  description = <<-EOT
